...
export function Sidebar() {
  ...
  return (
    <div className="fixed left-0 top-0 w-52 bg-white border-r border-slate-200 min-h-screen flex flex-col shadow-sm z-40">
      ...
      {/* Navigation */}
      <nav className="p-4 pt-3 space-y-1 flex-1">
        {navItems.map(({ path, label }) => (
          <Link 
            key={path} 
            href={path}
            className={cn(
              "relative group block px-3 py-2.5 text-sm font-medium rounded-lg cursor-pointer overflow-hidden transition-all duration-300 ease-in-out transform hover:scale-[1.02] whitespace-nowrap",
              location === path
